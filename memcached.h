...
struct settings {
    ...
    int num_instances;       /* number of instances of memory manager */
...
enum delta_result_type do_add_delta(conn *c, const char *key,
                                    ...
                                    uint64_t *cas, const uint32_t hv, const int instance_id);
enum store_item_type do_store_item(item *item, int comm, conn* c, const uint32_t hv, const int instance_id);
...
void thread_init(int nthreads, int instances_num, struct event_base *main_base);
...
item *item_alloc(char *key, size_t nkey, int flags, rel_time_t exptime, int nbytes, const int instance_id);
...
int   item_replace(item *it, item *new_it, const uint32_t hv, const int instance_id);
